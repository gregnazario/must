((): void => {
  ...
  interface TomlFieldMap {
    [key: string]: string;
  }

  interface TomlSectionMap {
    [section: string]: TomlFieldMap;
  }

  interface ParsedToml {
    sections: TomlSectionMap;
    errors: ValidationError[];
  }

  const KNOWN_RECIPE_TYPES: readonly string[] = [
    "shell",
    "rust-bin",
    "rust-lib",
    "rust-test",
  ...
  const VALID_CACHE_STRATEGIES: readonly string[] = ["hash", "mtime", "none"];
  ...
  function parseToml(input: string): ParsedToml {
    const result: ParsedToml = { sections: {}, errors: [] };
  ...
  function validateToml(parsed: ParsedToml): ValidationResult {
  ...
  function formatMessages(items: ValidationError[], max: number): string {
    if (items.length <= max) {
  ...
  function createValidator(container: HTMLElement): void {
  ...
  function init(): void {
    document.querySelectorAll<HTMLElement>("[data-must-validator]").forEach((el) => {
      createValidator(el);
    });
  }
